...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        <h2 class="text-xl font-bold text-gray-800">Data Ruangan</h2>
        <p class="text-sm text-gray-500 mt-0.5">Kelola data ruangan, laboratorium, dan fasilitas</p>
    </div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-5">
    <div class="flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            ...
            <input type="text" id="search-input" placeholder="Cari nama atau kode ruangan..." class="w-full pl-10 pr-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all">
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Ruangan</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Kode</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Kapasitas</th>
...
<x-modal id="modal-form" title="Form Ruangan">
    <form id="main-form" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" id="input-nama" placeholder="Contoh: Ruang Kelas 1" required class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Kode Ruangan <span class="text-red-500">*</span></label>
            <input type="text" id="input-kode" placeholder="Contoh: R01" required class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Kapasitas (Orang) <span class="text-red-500">*</span></label>
            <input type="number" id="input-kapasitas" placeholder="Contoh: 36" required class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
...
<script>
    ...
    async function fetchData(page = 1) {
        ...
        try {
            const url = new URL(window.location.origin + '/api/ruangans');
        ...
        data.forEach((item, index) => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-gray-50/70 transition-colors">
                    ...
                    <td class="px-5 py-4 font-semibold text-gray-800">${item.nama_ruangan}</td>
                    <td class="px-5 py-4"><span class="font-mono text-xs font-bold px-2 py-1 bg-gray-100 rounded text-gray-600">${item.kode_ruangan}</span></td>
                    <td class="px-5 py-4 text-gray-500 text-xs">${item.kapasitas} Orang</td>
    ...
    function openModalTambah() { document.getElementById('main-form').reset(); document.getElementById('item_id').value = ''; document.getElementById('modal-form-title').innerText = "Tambah Ruangan Baru"; openModal('modal-form'); }

    async function editData(id) {
        try {
            const res = await fetch(`/api/ruangans/${id}`, { headers: { 'Accept': 'application/json' } });
            const response = await res.json();
            if(response.success) {
                const item = response.data;
                document.getElementById('item_id').value = item.id;
                document.getElementById('input-nama').value = item.nama_ruangan;
                document.getElementById('input-kode').value = item.kode_ruangan;
                ...
                document.getElementById('modal-form-title').innerText = "Edit Ruangan";
    ...
    async function saveData() {
        ...
        const url = id ? `/api/ruangans/${id}` : `/api/ruangans`;
        const payload = { nama_ruangan: document.getElementById('input-nama').value, kode_ruangan: document.getElementById('input-kode').value, kapasitas: document.getElementById('input-kapasitas').value, status: document.querySelector(`input[name="input_status"]:checked`).value };
        try {
    ...
    async function deleteData(id, name) {
        if (!confirm(`Hapus data ruangan "${name}"?`)) return;
        try {
            const res = await fetch(`/api/ruangans/${id}`, { method: 'DELETE', headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfToken } });
            const data = await res.json();
