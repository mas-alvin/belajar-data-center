...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Kelola data kompetensi keahlian / jurusan</p>
    </div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-5">
    <div class="flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            ...
            <input type="text" id="search-input" placeholder="Cari nama atau kode jurusan..."
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Jurusan</th>
...
{{-- Modal Form --}}
<x-modal id="modal-form" title="Form Jurusan">
    <form id="main-form" class="space-y-4">
        <input type="hidden" id="item_id" value="">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Jurusan <span class="text-red-500">*</span></label>
            <input type="text" id="input-nama" placeholder="Contoh: Teknik Komputer dan Jaringan" required
                   class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Kode Jurusan <span class="text-red-500">*</span></label>
            <input type="text" id="input-kode" placeholder="Contoh: TKJ" required
                   class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
...
<script>
    ...
    async function fetchData(page = 1) {
        ...
        try {
            const url = new URL(window.location.origin + '/api/jurusans');
            ...
            tbody.innerHTML = `<tr><td colspan="5" class="px-5 py-4 text-center text-red-500">Gagal mengambil data.</td></tr>`;
        }
    }
    ...
    function renderTable(data) {
        tbody.innerHTML = '';
        if (data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="5" class="px-5 py-8 text-center text-gray-500">Tidak ada data ditemukan.</td></tr>`;
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-gray-50/70 transition-colors">
                    ...
                    <td class="px-5 py-4 font-semibold text-gray-800">${item.nama_jurusan}</td>
                    <td class="px-5 py-4"><span class="font-mono text-xs font-bold px-2 py-1 bg-gray-100 rounded text-gray-600">${item.kode_jurusan}</span></td>
    ...
    function openModalTambah() {
        ...
        document.getElementById('modal-form-title').innerText = "Tambah Jurusan Baru";
        openModal('modal-form');
    }

    async function editData(id) {
        try {
            const res = await fetch(`/api/jurusans/${id}`, { headers: { 'Accept': 'application/json' } });
            const response = await res.json();
            if(response.success) {
                const item = response.data;
                document.getElementById('item_id').value = item.id;
                document.getElementById('input-nama').value = item.nama_jurusan;
                document.getElementById('input-kode').value = item.kode_jurusan;
                document.querySelector(`input[name="input_status"][value="${item.status}"]`).checked = true;
                document.getElementById('modal-form-title').innerText = "Edit Data Jurusan";
                openModal('modal-form');
            }
    ...
    async function saveData() {
        ...
        const url = id ? `/api/jurusans/${id}` : `/api/jurusans`;
        const payload = {
            nama_jurusan: document.getElementById('input-nama').value,
            kode_jurusan: document.getElementById('input-kode').value,
    ...
    async function deleteData(id, name) {
        if (!confirm(`Hapus data jurusan "${name}"?`)) return;
        try {
