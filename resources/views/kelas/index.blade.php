...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Kelola data kelas dan wali kelas</p>
    </div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-5">
    <div class="flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            ...
            <input type="text" id="search-input" placeholder="Cari nama kelas..."
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Kelas</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Jurusan</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Wali Kelas</th>
...
{{-- Modal Form --}}
<x-modal id="modal-form" title="Form Kelas">
    <form id="main-form" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" id="input-nama" placeholder="Contoh: XII MIPA 1" required
        ...
        <div>
            ...
            <select id="input-jurusan" required class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
                <option value="">Pilih Jurusan...</option>
            </select>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Wali Kelas</label>
            <select id="input-wali" class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
...
<script>
    ...
    async function loadOptions() {
        try {
            const [jurusansRes, gurusRes] = await Promise.all([
                fetch('/api/jurusans?per_page=100', { headers: { 'Accept': 'application/json' } }),
                fetch('/api/gurus?per_page=100', { headers: { 'Accept': 'application/json' } })
            ]);
            const [jurusans, gurus] = await Promise.all([jurusansRes.json(), gurusRes.json()]);
            
            if(jurusans.success) {
                const jurusanSelect = document.getElementById('input-jurusan');
                jurusans.data.forEach(item => {
                    jurusanSelect.innerHTML += `<option value="${item.id}">${item.nama_jurusan}</option>`;
                });
            }
            if(gurus.success) {
                const guruSelect = document.getElementById('input-wali');
                gurus.data.forEach(item => {
                    guruSelect.innerHTML += `<option value="${item.id}">${item.nama_guru}</option>`;
                });
            }
        } catch (e) {
            console.error(e);
    ...
    function renderTable(data) {
        tbody.innerHTML = '';
        if (data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="px-5 py-8 text-center text-gray-500">Tidak ada data ditemukan.</td></tr>`;
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-gray-50/70 transition-colors">
                    ...
                    <td class="px-5 py-4 font-semibold text-gray-800">${item.nama_kelas}</td>
                    <td class="px-5 py-4 text-gray-600">${item.jurusan ? item.jurusan.nama_jurusan : '-'}</td>
                    <td class="px-5 py-4 text-gray-600">${item.wali_kelas ? item.wali_kelas.nama_guru : '-'}</td>
    ...
    function openModalTambah() {
        ...
        document.getElementById('modal-form-title').innerText = "Tambah Kelas Baru";
    ...
    async function editData(id) {
        try {
            ...
            if(response.success) {
                ...
                document.getElementById('input-nama').value = item.nama_kelas;
                document.getElementById('input-jurusan').value = item.jurusan_id || '';
                document.getElementById('input-wali').value = item.wali_kelas_id || '';
                document.querySelector(`input[name="input_status"][value="${item.status}"]`).checked = true;
                document.getElementById('modal-form-title').innerText = "Edit Data Kelas";
                openModal('modal-form');
            }
    ...
    async function saveData() {
        ...
        const payload = {
            ...
            wali_kelas_id: document.getElementById('input-wali').value,
