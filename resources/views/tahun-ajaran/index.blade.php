...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        <h2 class="text-xl font-bold text-gray-800">Tahun Ajaran</h2>
        <p class="text-sm text-gray-500 mt-0.5">Kelola periode tahun ajaran dan semester aktif</p>
    </div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-5">
    <div class="flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            ...
            <input type="text" id="search-input" placeholder="Cari tahun ajaran..." class="w-full pl-10 pr-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all">
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tahun Ajaran</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Semester</th>
            ...
            <tbody id="data-tbody" class="divide-y divide-gray-50">
                <tr><td colspan="5" class="px-5 py-8 text-center text-gray-400">Memuat data...</td></tr>
            </tbody>
        </table>
    </div>
...
<x-modal id="modal-form" title="Form Tahun Ajaran">
    <form id="main-form" class="space-y-4">
        <input type="hidden" id="item_id" value="">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Tahun Ajaran <span class="text-red-500">*</span></label>
            <input type="text" id="input-tahun" placeholder="Contoh: 2024/2025" required class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Semester <span class="text-red-500">*</span></label>
            <select id="input-semester" required class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all appearance-none">
...
<script>
    ...
    async function fetchData(page = 1) {
        ...
        tbody.innerHTML = `<tr><td colspan="5" class="px-5 py-8 text-center text-gray-400">Loading data...</td></tr>`;
        try {
            const url = new URL(window.location.origin + '/api/tahun-ajarans');
    ...
    function renderTable(data) {
        tbody.innerHTML = '';
        if (data.length === 0) { tbody.innerHTML = `<tr><td colspan="5" class="px-5 py-8 text-center text-gray-500">Tidak ada data ditemukan.</td></tr>`; return; }
        data.forEach((item, index) => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-gray-50/70 transition-colors">
                    ...
                    <td class="px-5 py-4 font-semibold text-gray-800">${item.tahun}</td>
                    <td class="px-5 py-4 text-gray-600">${item.semester}</td>
    ...
    function openModalTambah() { document.getElementById('main-form').reset(); document.getElementById('item_id').value = ''; document.getElementById('modal-form-title').innerText = "Tambah Tahun Ajaran Baru"; openModal('modal-form'); }

    async function editData(id) {
        try {
            const res = await fetch(`/api/tahun-ajarans/${id}`, { headers: { 'Accept': 'application/json' } });
            const response = await res.json();
            if(response.success) {
                const item = response.data;
                document.getElementById('item_id').value = item.id;
                document.getElementById('input-tahun').value = item.tahun;
                document.getElementById('input-semester').value = item.semester;
    ...
    async function saveData() {
        ...
        const url = id ? `/api/tahun-ajarans/${id}` : `/api/tahun-ajarans`;
        const payload = { tahun: document.getElementById('input-tahun').value, semester: document.getElementById('input-semester').value, status: document.querySelector(`input[name="input_status"]:checked`).value };
        try {
    ...
    async function deleteData(id, name) {
        if (!confirm(`Hapus data tahun ajaran "${name}"?`)) return;
        try {
            const res = await fetch(`/api/tahun-ajarans/${id}`, { method: 'DELETE', headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfToken } });
            const data = await res.json();
