...
@section('page-title', 'Data Master Guru')
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-5">
    <div class="flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            ...
            <input type="text" id="search-input" placeholder="Cari NIP atau nama guru..."
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">NIP</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Guru</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">L/P</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">No. Telp</th>
            ...
            <tbody id="data-tbody" class="divide-y divide-gray-50">
                <tr>
                    <td colspan="7" class="px-5 py-8 text-center text-gray-400">Memuat data...</td>
                </tr>
            </tbody>
        </table>
    </div>
...
{{-- Modal Form --}}
<x-modal id="modal-form" title="Form Guru">
    <form id="main-form" class="space-y-4">
        <input type="hidden" id="item_id" value="">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">NIP</label>
            <input type="text" id="input-nip" placeholder="Contoh: 198001012010011001"
                   class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Guru <span class="text-red-500">*</span></label>
            <input type="text" id="input-nama" placeholder="Contoh: Budi Santoso, S.Pd" required
                   class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Jenis Kelamin <span class="text-red-500">*</span></label>
            <select id="input-jk" required class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
                <option value="L">Laki-laki</option>
                <option value="P">Perempuan</option>
            </select>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">No. Telp</label>
            <input type="text" id="input-telp" placeholder="Contoh: 081234567890"
                   class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all">
        </div>
        <div>
...
<script>
    ...
    async function fetchData(page = 1) {
        ...
        tbody.innerHTML = `<tr><td colspan="7" class="px-5 py-8 text-center text-gray-400">Loading data...</td></tr>`;

        try {
            const url = new URL(window.location.origin + '/api/gurus');
            ...
            tbody.innerHTML = `<tr><td colspan="7" class="px-5 py-4 text-center text-red-500">Gagal mengambil data.</td></tr>`;
        }
    }
    ...
    function renderTable(data) {
        tbody.innerHTML = '';
        if (data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" class="px-5 py-8 text-center text-gray-500">Tidak ada data ditemukan.</td></tr>`;
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-gray-50/70 transition-colors">
                    ...
                    <td class="px-5 py-4 font-semibold text-gray-800">${item.nip || '-'}</td>
                    <td class="px-5 py-4 text-gray-600">${item.nama_guru}</td>
                    <td class="px-5 py-4 text-gray-600">${item.jenis_kelamin}</td>
                    <td class="px-5 py-4 text-gray-600">${item.no_telp || '-'}</td>
    ...
    function openModalTambah() {
        ...
        document.getElementById('modal-form-title').innerText = "Tambah Guru Baru";
        openModal('modal-form');
    }

    async function editData(id) {
        try {
            const res = await fetch(`/api/gurus/${id}`, { headers: { 'Accept': 'application/json' } });
            const response = await res.json();
            if(response.success) {
                const item = response.data;
                document.getElementById('item_id').value = item.id;
                document.getElementById('input-nip').value = item.nip || '';
                document.getElementById('input-nama').value = item.nama_guru;
                document.getElementById('input-jk').value = item.jenis_kelamin;
                document.getElementById('input-telp').value = item.no_telp || '';
                document.querySelector(`input[name="input_status"][value="${item.status}"]`).checked = true;
                document.getElementById('modal-form-title').innerText = "Edit Data Guru";
                openModal('modal-form');
            }
    ...
    async function saveData() {
        ...
        const url = id ? `/api/gurus/${id}` : `/api/gurus`;
        const payload = {
            nip: document.getElementById('input-nip').value,
            nama_guru: document.getElementById('input-nama').value,
            jenis_kelamin: document.getElementById('input-jk').value,
            no_telp: document.getElementById('input-telp').value,
    ...
    async function deleteData(id, name) {
        ...
        try {
            const res = await fetch(`/api/gurus/${id}`, {
                method: 'DELETE',
